...
export function SpotifySettings() {
  ...
  useEffect(() => {
    ...
    fetch("/credentials/spotify/credentials")
      .then((res) => res.json())
      .then((data) => {
  ...
  const handleSave = async () => {
    ...
    setIsSaving(true);
    await fetch("/credentials/spotify/set", {
      method: "POST",
  ...
  const handleAuthenticate = async () => {
    await handleSave(); // Save credentials if not already saved

    const response = await fetch("/auth/spotify/authorize", { method: "GET" });
    const data = await response.json();

    if (data.message === "✅ Spotify already authorized.") {
      alert(data.message); // Show a message if already authorized
  ...
  const handleDelete = async () => {
    const confirmed = window.confirm("Are you sure you want to delete Spotify credentials?");
    if (!confirmed) return;

    const response = await fetch("/credentials/spotify/delete", { method: "DELETE" });
    const data = await response.json();
